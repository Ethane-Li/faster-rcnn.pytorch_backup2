...
def prepare_w2v(data_path, image_set):
    '''
    generate dictionary for word vectors 
    key: class name
    value: word vector for corresponding class name
    '''
    if image_set in ["minitrain","train"]:
        w2v_cat = 'seen'
    if image_set == "test":
        w2v_cat = 'unseen'
    cls_root = os.path.join(data_path, 'zsd', 'seen_unseen_classes', 'vg_' + w2v_cat + '_classes.json' )
    w2v_root = os.path.join(data_path, 'zsd', 'cls_vec' )
    with open(cls_root) as f:
        _clses = json.load(f)
    w2v_content = dict()
    for _cls in _clses:
        w2v_content[_cls] = np.load(os.path.join(w2v_root, _cls + '.npy'))
...
class vg(imdb):
    def __init__(self, version, image_set, ):
        ...
        #     count += 1
        # Load Word Vectors
        self.name_to_w2v = prepare_w2v(self._data_path, self._image_set)
        ...
        if os.path.exists(os.path.join(self._data_path, 'with_attr', "vg_image_index_{}.p".format(self._image_set))):
            with open(os.path.join(self._data_path,'with_attr', "vg_image_index_{}.p".format(self._image_set)), 'rb') as fp:
                self._image_index = pickle.load(fp)
            load_index_from_file = True

        load_id_from_file = False
        if os.path.exists(os.path.join(self._data_path, 'with_attr', "vg_id_to_dir_{}.p".format(self._image_set))):
            with open(os.path.join(self._data_path, 'with_attr', "vg_id_to_dir_{}.p".format(self._image_set)), 'rb') as fp:
                self._id_to_dir = pickle.load(fp)
            load_id_from_file = True

        if not load_index_from_file or not load_id_from_file:
            print('write new .p file')
            self._image_index, self._id_to_dir = self._load_image_set_index()
            with open(os.path.join(self._data_path, 'with_attr', "vg_image_index_{}.p".format(self._image_set)), 'wb') as fp:
                pickle.dump(self._image_index, fp)
            with open(os.path.join(self._data_path, 'with_attr', "vg_id_to_dir_{}.p".format(self._image_set)), 'wb') as fp:
    ...
    def _load_image_set_index(self):
        ...
        with open(training_split_file) as f:
          ...
          print('length of metadata: ' +str(len(metadata)))
          if self._image_set == "minitrain":
    ...
    def _annotation_path(self, index):
        ...
        return os.path.join(self._data_path, 'zsd_xml', str(index) + '.xml')
    ...
    def _load_vg_annotation(self, index):
        ...
        gt_wordvectors = np.zeros((num_objs, 100),dtype=np.int32)
        # gt_attributes = np.zeros((num_objs, 16), dtype=np.int32)
        overlaps = np.zeros((num_objs, self.num_classes), dtype=np.float32)
        # "Seg" area for pascal is just the box area
        seg_areas = np.zeros((num_objs), dtype=np.float32)

        # Load object bounding boxes into a data frame. self.name_to_w2v
        obj_dict = {}
        ix = 0
        for obj in objs:
            obj_name = obj.find('name').text.lower().strip()
            if obj_name in self._class_to_ind:
                ...
                # ground truth for word2vec
                gt_wordvectors[ix] = self.name_to_w2v[obj_name]
                ix += 1
        # clip gt_classes and gt_relations
        gt_classes = gt_classes[:ix]
        gt_wordvectors = gt_wordvectors[:ix]
        ...
        # print(gt_wordvectors)

        

        return {'boxes' : boxes,
                'gt_classes': gt_classes,
                'gt_wordvectors': gt_wordvectors,
